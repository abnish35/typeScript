...
// Array type generics
const names: Array<string | number> = [] // similar to string[]
//names[0].split(" ");
...
function merges<T, U>(obj1: T, obj2: U){
    return Object.assign(obj1, obj2)
}
...
class DataStorage<T extends string | number | boolean>{
    ...
    removeItem(item: T){
        if(this.data.indexOf (item) === -1){ // for object checking
            return;
        }
...
let printStorage = new DataStorage<string>()
...
function createCourseGoal(title: string, description: string, startingDate: Date )
: CourseGoal{
...
const namess: Readonly<string[]> = ["Abnish", "Alok"]
// namess.push("Amit")
const num: Readonly<number[]> = [12, 45]
// num.push(16)
